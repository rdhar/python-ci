...
def add_testcases(jason_dict):
    ...
    if len(test_suite) > 0:
       document = ET.Element('testSuite', errors = str(error_count), failures = str(failure_count), name = "JSON Translate", skipped = str(skip_count), tests = str(len(test_suite)) , time = "This needs to be added")
    i = 0
    while i < len(test_suite):

        test_case_name_array = test_case_processor( test_suite[i]["name"])

        element = ET.SubElement(document, 'testCase',classname = test_case_name_array[0] + "." + test_case_name_array[1], file =  test_case_name_array[0], name = test_case_name_array[2], time = str(test_suite[i]["teardown"] ["duration"]) )
